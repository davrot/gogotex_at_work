...
IMAGE=realtime-go:integration
CONTAINER=realtime-go-integration
PORT=8088
...
PG_CONTAINER=realtime-pg-integration
PG_PORT=5439
NETWORK=realtime-integ-net
...
docker run -d --name $PG_CONTAINER --network $NETWORK -e POSTGRES_PASSWORD=pass -e POSTGRES_DB=realtime -p ${PG_PORT}:5432 postgres:15-alpine

# wait for Postgres
for i in {1..20}; do
  if docker run --network $NETWORK --rm postgres:15-alpine psql postgresql://postgres:pass@${PG_CONTAINER}:5432/realtime -c '\l' >/dev/null 2>&1; then
    echo "postgres ok"
    break
  fi
...
docker run --network $NETWORK --rm postgres:15-alpine psql postgresql://postgres:pass@${PG_CONTAINER}:5432/realtime -c "CREATE EXTENSION IF NOT EXISTS pgcrypto;" || true
docker run --network $NETWORK --rm postgres:15-alpine psql postgresql://postgres:pass@${PG_CONTAINER}:5432/realtime -c "CREATE TABLE IF NOT EXISTS messages (id UUID PRIMARY KEY, channel TEXT NOT NULL, body TEXT NOT NULL, created_at BIGINT NOT NULL);" || true

# Run the service in postgres mode on the same network
docker rm -f "$CONTAINER" 2>/dev/null || true
docker run -d --name "$CONTAINER" --network $NETWORK -p ${PORT}:8080 -e STORE=postgres -e DATABASE_URL=postgres://postgres:pass@${PG_CONTAINER}:5432/realtime?sslmode=disable "$IMAGE"
...
# Run a publish/list check in postgres mode
...
# publish a message and assert it's visible in list
create_code=$(timeout 30s docker run --network container:$CONTAINER --rm curlimages/curl:latest -sS -o /dev/null -w "%{http_code}" -H 'Content-Type: application/json' -d '{"channel":"chan1","body":"hello","id":"11111111-1111-1111-1111-111111111111"}' http://localhost:8080/messages) || true
if [ "$create_code" != "201" ]; then
  echo "postgres messages publish failed (code: $create_code)"
  docker logs "$CONTAINER" || true
...
if ! docker run --network container:$CONTAINER --rm curlimages/curl:latest -sS http://localhost:8080/messages | grep -q hello; then
  echo "postgres messages list missing created message"
  docker logs "$CONTAINER" || true
  docker rm -f "$CONTAINER" || true
  docker rm -f $PG_CONTAINER || true
  exit 8
