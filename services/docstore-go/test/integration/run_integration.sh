...
create_code=$(timeout 30s docker run --network container:$CONTAINER --rm curlimages/curl:latest -sS -o /dev/null -w "%{http_code}" -H 'Content-Type: application/json' -d '{"title":"DBIntegration","body":"dbtest","id":"11111111-1111-1111-1111-111111111111"}' http://localhost:8080/documents) || true
