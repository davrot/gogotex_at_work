...
IMAGE=project-history-go:integration
CONTAINER=project-history-go-integration
PORT=8086
...
PG_PORT=5437
NETWORK=project-history-integ-net
...
docker run -d --name $PG_CONTAINER --network $NETWORK -e POSTGRES_PASSWORD=pass -e POSTGRES_DB=project_history -p ${PG_PORT}:5432 postgres:15-alpine

# wait for Postgres
for i in {1..20}; do
  if docker run --network $NETWORK --rm postgres:15-alpine psql postgresql://postgres:pass@${PG_CONTAINER}:5432/project_history -c '\l' >/dev/null 2>&1; then
    echo "postgres ok"
    break
  fi
...
docker run --network $NETWORK --rm postgres:15-alpine psql postgresql://postgres:pass@${PG_CONTAINER}:5432/project_history -c "CREATE EXTENSION IF NOT EXISTS pgcrypto;" || true
docker run --network $NETWORK --rm postgres:15-alpine psql postgresql://postgres:pass@${PG_CONTAINER}:5432/project_history -c "CREATE TABLE IF NOT EXISTS events (id UUID PRIMARY KEY, project_id UUID NOT NULL, type TEXT NOT NULL, payload TEXT, created_at BIGINT NOT NULL);" || true

# Run the service in postgres mode on the same network
docker rm -f "$CONTAINER" 2>/dev/null || true
docker run -d --name "$CONTAINER" --network $NETWORK -p ${PORT}:8080 -e STORE=postgres -e DATABASE_URL=postgres://postgres:pass@${PG_CONTAINER}:5432/project_history?sslmode=disable "$IMAGE"
...
if ! docker run --network container:$CONTAINER --rm curlimages/curl:latest -sS http://localhost:8080/events | grep -q dbtest; then
  echo "postgres events list missing created event"
  docker logs "$CONTAINER" || true
  docker rm -f "$CONTAINER" || true
  docker rm -f $PG_CONTAINER || true
  exit 8
