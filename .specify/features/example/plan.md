...
- The key→user lookup micro-benchmark (T026) must specify a runner profile (recommended: 2 vCPU, 4GB RAM) and reproducible harness parameters; include baseline artifact upload to CI for historical comparison. The canonical runner profile and dataset hints are defined in `ci/benchmarks/harness-config.json` which should be the single source of truth referenced by CI jobs and documentation. Add a companion introspection micro-benchmark (T026b) that measures introspection p50/p95/p99 for both local introspection and OAuth2 fallback paths; this job MUST be gated in CI and use the same runner profile for comparability.
