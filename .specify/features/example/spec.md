...
- Introspection: token introspection latency p95 ≤ 100ms in normal conditions; introspection endpoints should be instrumented and benchmarked in CI.
  - Measurement harness and "normal conditions": see the general performance harness above. For clarity, **"normal conditions"** are defined by the canonical benchmark harness configuration in `ci/benchmarks/harness-config.json` (recommended runner profile: **2 vCPU, 4GB RAM**, seeded dataset as documented). Introspection benchmarks MUST be run for local introspection and OAuth2 fallback paths under representative load; CI MUST publish p50/p95/p99 and fail jobs when thresholds are exceeded. Add task T026b for introspection micro-benchmark coverage.
...
- Revoked token or key: cache invalidation must reflect revocation immediately. Implementations MUST ensure that after a successful revoke/delete request returns, subsequent introspection requests return `active: false` within **500ms** under normal operating conditions. To support this requirement, services MUST publish an invalidation event (for example to `auth.cache.invalidate` or an equivalent invalidation collection) and synchronously purge or invalidate any in-process caches before returning success. Contract and integration tests MUST assert the 500ms bound and the presence of an invalidation message. If for operational reasons a short observable window is unavoidable, document the exact maximum window in the feature README and include a migration plan and rationale.
...
- Revocation immediacy and acceptable stale windows:
  - Acceptable stale window for _non-urgent_ invalidations: **60s** (configurable, default).
