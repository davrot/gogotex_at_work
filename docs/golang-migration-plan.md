...
All migrated Go services and components will be labeled **GoGoTeX** in documentation, service names, and build artifacts. This label should be used in internal docs, PR titles (e.g., "migrate git-bridge to GoGoTeX"), Docker image tags (e.g., `develop-gogotex-git-bridge:latest`), and CI job names where appropriate to distinguish migrated components from legacy implementations.
...
- Contract tests pass against Node _and_ Go (same status codes, response shapes, and edge-case semantics).
