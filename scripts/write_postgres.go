...
`
	if err := os.WriteFile("services/docstore-go/internal/store/postgres_integration_test.go", []byte(content), 0644); err != nil {
		log.Fatalf("write failed: %v", err)
	}
	log.Printf("wrote postgres_integration_test.go")
}
